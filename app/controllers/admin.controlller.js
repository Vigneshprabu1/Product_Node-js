...
exports.findAll = (req, res) => {
    ...
    .then(orders => {
        ...
        Order.count({updatedAt : { $gte : (new Date())}, order_status: 'delivered'})
        .then(delivered => {
    ...
    Order.count({updatedAt : { $gte : (new Date())}, order_status: 'dispatched'})
    .then(dispatched => {
        console.log("todayOrderDispatched: ",dispatched);
        totalCount.todayOrderDispatched=dispatched;
        //res.send(totalCount);

    Order.count({updatedAt : { $gte : (new Date())}, order_status: 'cancelled'})
    .then(cancelled => {
